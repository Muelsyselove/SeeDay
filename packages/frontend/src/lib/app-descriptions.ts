...
export function getAppDescription(appName: string, displayTitle?: string, music?: { title?: string; artist?: string; app?: string }, isForeground?: boolean): string {
  ...
  if (!base) {
    ...
    if (displayTitle) {
      base = isForeground ? `正在查看「${displayTitle}」喵~` : `正在运行「${displayTitle}」喵~`;
    } else {
      base = isForeground ? "正在查看别的东西喵~" : "正在运行别的程序喵~";
    }
  }
