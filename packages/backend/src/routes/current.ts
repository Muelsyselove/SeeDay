import { getAllDeviceStates, getRecentActivities, getAllDeviceAppStates } from "../db";
...
export function handleCurrent(clientIp: string, userAgent?: string): Response {
  ...
  const deviceAppStates = getAllDeviceAppStates.all() as DeviceState[];
  const recentActivities = getRecentActivities.all() as ActivityRecord[];

  return Response.json({
    devices: preparePublicDevices(devices),
    device_app_states: preparePublicDevices(deviceAppStates),
