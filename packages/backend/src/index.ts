...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    try {
      ...
      } else if (pathname === "/api/daily-summary/generate" && req.method === "POST") {
        response = await handleGenerateDailySummary(req);
      } else if (pathname === "/api/daily-summary/debug" && req.method === "GET") {
        response = handleDailySummaryDebug(url);
