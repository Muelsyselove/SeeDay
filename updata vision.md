...
- 创建 `v1.5.13-release` tag 触发包含修复的 Release 构建流程

### 修复时间轴功能不显示手机端数据问题
- **问题**：Debug 版本构建成功，前端可以看到手机在线并运行界面，但时间轴功能无法显示手机端的真实数据，仅显示之前 curl 测试写入的 3 条记录
- **原因**：时间轴查询使用了 SQLite 的 `datetime(?, '-2 days')` 函数，将日期字符串当作 UTC 处理，而数据库中 `started_at` 存储的是设备本地时间字符串（如 "2026-04-20 20:15:00"），导致查询范围不匹配
- **修复**：重写时间轴查询逻辑，直接使用本地日期字符串进行范围查询（`started_at >= ? AND started_at <= ?`），扩大查询范围至前2天至后1天以捕获跨日数据
- **客户端改进**：`ApiClient` 在上报请求中添加 `tz` 字段，发送设备时区偏移（UTC+8 为 -480 分钟），为未来时区处理做准备
- **服务端改进**：报告端点直接解析设备发送的本地时间戳字符串，保持与现有数据格式一致
